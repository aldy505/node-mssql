...
const { tracer, getSpanName: getOperationName, getCollectionName } = require('../tracer')
...
class Request extends EventEmitter {
  ...
  query (command, callback) {
    ...
    if (typeof callback === 'function') {
      return tracer.startActiveSpan(command, {
        kind: SpanKind.CLIENT,
        attributes: {
          'db.system': 'mssql',
          'db.operation': getOperationName(command),
          'db.operation.name': getOperationName(command),
          'db.collection.name': getCollectionName(command),
          'db.query.text': command
        }
      }, (span) => {
        if (this?.config?.port) {
          span.setAttribute('server.port', this.config.port)
        }
        if (this?.config?.server) {
          span.setAttribute('server.address', this.config.server)
        }
